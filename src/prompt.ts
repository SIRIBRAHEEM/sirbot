...
// Lines are queued as they arrive rather than read via rl.question().
const queue: string[] = [];
let waiter: ((line: string) => void) | null = null;

// Terminal mode only when stdin really is a TTY.
...
export async function askChoice<T>(
  ...
  for (;;) {
    const raw = await ask(
      chalk.gray(`  › choose 1-${choices.length} [${defaultIndex + 1}]: `),
    ...
    console.log(chalk.red(`  ✗ Enter a number between 1 and ${choices.length}.`));
  }
}
...
export async function askYesNo(question: string, defaultYes = false): Promise<boolean> {
  ...
  for (;;) {
    ...
    console.log(chalk.red("  ✗ Answer y or n."));
  }
}
